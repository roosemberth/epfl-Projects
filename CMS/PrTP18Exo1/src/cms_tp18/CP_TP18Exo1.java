...
public class CP_TP18Exo1 {
	...
	public static void main(String[] args) throws IOException{
		...
		int nEtudiants = 1;
		// Throws IOException
		DataOutputStream RecordsFile = new DataOutputStream(new FileOutputStream("notes.dat"));
		do {
			System.out.println("Introduisez le nom du " + nEtudiants + (nEtudiants==1?"ére":"éme") + " étudiant. " );
			RecordsFile.writeUTF(scan.nextLine());
			for (int c=0; c<4; ++c){
				System.out.println("Introduisez les notes du contrôle numero  "+ (c+1) );
				RecordsFile.writeDouble(scan.nextDouble());
			}
